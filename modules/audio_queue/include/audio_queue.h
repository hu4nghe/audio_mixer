...
template <audio_sample_type AudioType> struct audio_queue
{
    /**
    ...
    audio_queue()
        : m_queue(
              static_cast<size_t>(m_expected_context.m_channel_num) * m_expected_context.m_sample_rate *
     ...
     */
    audio_queue(
        audio_context user_expected_ctx,
        size_t        user_expected_lat_ms = 200)
        : m_expected_context(user_expected_ctx),
          m_queue(
              static_cast<size_t>(user_expected_ctx.m_channel_num) * user_expected_ctx.m_sample_rate *
              user_expected_lat_ms / 1000)
    {}
     ...
     */
    bool push_audio(
        const audio_context& input_context,
        AudioType*           input_data,
        std::size_t          input_frame)
    {
        ...
        auto input_data_float = std::span{input_data, input_frame * input_channels} | std::views::transform(to_float) |
        ...
        if (auto matrix_opt = m_expected_context.need_conversion(input_context))
        {
            ...
            for (size_t frame_idx = 0; frame_idx < temp_frame; frame_idx++)
            {
                ...
                for (const auto& row : convert_matrix)
                    out_frame[&row - convert_matrix.data()] =
                        std::inner_product(row.begin(), row.end(), in_frame, 0.0F);
            }
        }
        ...
        for (const auto sample : output_audio.empty() ? temp : output_audio)
            if (!m_queue.enqueue(sample)) drops++;
     ...
     */
    bool pop_audio(
        const audio_context& output_ctx,
        AudioType*           output_buffer,
